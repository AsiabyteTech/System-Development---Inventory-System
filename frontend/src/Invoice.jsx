      ...
      <div className='flex-1 ml-16 md:ml-64 transition-all duration-300'>
        <main className="all-main-content">
          ...
          <div className="page-banner flex justify-center items-center mb-8">
            <h2 className="bg-[#00008B] text-white px-12 py-2 rounded-full text-xl font-bold shadow-md">Invoice</h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <button 
              ...
              className="group bg-gradient-to-br from-blue-900 to-blue-700 rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 overflow-hidden"
            >
              <div className="p-6">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-medium text-blue-100 uppercase tracking-wider">SUPPLIER</span>
                  </div>
                </div>
                <div className="space-y-2">
                  <h3 className="text-2xl font-bold text-white">Supplier Inc.</h3>
                  <p className="text-sm text-blue-100">Primary vendor for networking equipment</p>
                </div>
              </div>
            </button>
          ...
          <p className="text-xs text-slate-400 mb-6 font-medium italic">*Recent</p>
          ...
          <div className="bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 p-6 mb-6 border border-slate-100">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
              <div className="lg:w-2/3">
                ...
                <p className="text-xs text-slate-400 mt-2">*Reference No, Supplier Name</p>
              </div>
          ...
          {/* Invoice Table - Modern Dashboard Style */}
          <div className="bg-white rounded-xl shadow-lg border border-slate-100 overflow-hidden">
            {filteredInvoices.length > 0 ? (
                ...
                <table className="w-full">
                  <thead>
                    <tr className="bg-gradient-to-r from-blue-900 to-blue-700 text-white">
                      ...
                      <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider">Invoice Date</th>
                      <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider">SN Details</th>
                      <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider">Supplier</th>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {filteredInvoices.map((inv) => (
                      <tr key={inv.id} className="hover:bg-blue-50/50 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <span className="font-semibold text-blue-900">{inv.refNo}</span>
                        </td>
                        <td className="px-6 py-4 text-slate-600">{inv.date}</td>
                        <td className="px-6 py-4">
                          <div className="flex flex-col space-y-1">
                            {inv.snDetails.map((sn, idx) => (
                              <span key={idx} className="inline-block bg-slate-100 text-slate-600 px-2 py-1 rounded text-xs w-fit">
                                {sn}
                              </span>
                            ))}
                          </div>
                        ...
                        <td className="px-6 py-4">
                          <span className="font-medium text-blue-900">{inv.supplier}</span>
                        </td>
                        <td className="px-6 py-4">
                          <span className="font-bold text-slate-800">RM {inv.amount}</span>
                        </td>
                        <td className="px-6 py-4">
              ...
              <div className="flex flex-col items-center justify-center py-20 px-4">
                ...
                <p className="text-slate-500 mb-4">Try searching for a different Reference No or Supplier</p>
                <button 
